...
class NetworkShareDisconnected(RuntimeError):
    """Raised when the SMB/network output disappears or becomes unwritable."""
...
def assert_output_share_alive(outdir: Path):
    ...
    if str(outdir).startswith("/Volumes/"):
        ...
        if len(parts) < 3:
            raise NetworkShareDisconnected(
                f"Invalid /Volumes output path: {outdir}"
            )
        ...
        if not outdir.exists():
            raise NetworkShareDisconnected(
                f"Benchmark output path disappeared: {outdir}"
            )
    ...
    try:
        ...
        raise NetworkShareDisconnected(
            f"Output share is no longer writable: {outdir}\n"
...
def build_sample(
    *,
    ...
    benchmark_root = tif_path.parents[2]
    assert_output_share_alive(benchmark_root)
...
def main():
    ...
    for i, row in work.iterrows():
        ...
        try:
            ...
            print("Completed valid TIFF/NPZ files remain intact.")
            print("Reconnect the SAME SMB mount path, then rerun the SAME command.")
            print("Existing valid outputs will be detected and skipped.")
            raise SystemExit(3)
