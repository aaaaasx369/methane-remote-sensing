...
# Known corrupted public-preview L3 asset encountered during the 156-positive audit.
# We exclude it from temporal-negative candidate search only; we do NOT drop
# the affected positive samples.
BAD_L3_COLLECTION_IDS = {"06CA05F0"}
BAD_L3_SYSTEM_INDEXES = {
    "MSAT_L3_45m_COG_GEE_c06CA05F0_p4696_v02000001_20250121T201240Z_201312Z"
}
...
def search_l3_candidates(row: pd.Series, positive_dt: pd.Timestamp, args):
    ...
    ic = ee.ImageCollection(L3_ASSET).filterBounds(point)

    # Exclude known corrupted official preview asset before any pixel operation.
    # Filtering before .map(add_qa) prevents one bad scene from aborting the
    # entire point search for every location that intersects its footprint.
    for bad_cid in BAD_L3_COLLECTION_IDS:
        ic = ic.filter(ee.Filter.neq("collection_id", bad_cid))
        ic = ic.filter(ee.Filter.neq("collection_id", f"c{bad_cid}"))
    for bad_index in BAD_L3_SYSTEM_INDEXES:
        ic = ic.filter(ee.Filter.neq("system:index", bad_index))
...
def main():
    ...
    print("Known corrupted L3 collection excluded from candidate search: 06CA05F0")
    print()
