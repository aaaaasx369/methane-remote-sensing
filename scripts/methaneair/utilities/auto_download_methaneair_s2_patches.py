...
# =========================
# Important:
# Use the fixed candidate file with lat/lon
# =========================
INPUT_CANDIDATES = [
...
def load_input_table():
    for path in INPUT_CANDIDATES:
        if not path.exists():
            print("File not found, skip:", path)
            continue

        df = pd.read_csv(path)

        print("\nTrying input file:", path)
        ...
        print("This file does not have enough lat/lon/time columns.")

    raise FileNotFoundError(
        "No usable controlled-release table found. "
        "Please run fix_controlled_release_candidate_latlon.py first."
    )
...
def add_label(df):
    ...
    for _, row in df.iterrows():
        ...
        if label is None and release_col is not None:
            label = parse_label(row[release_col])

        if label is None and emission_col is not None:
...
def filter_s2_available(df):
    ...
    print("Using S2 availability column:", s2_col)

    df = df.copy()

    # usable_s2 may be True/False; s2_count is numeric
    if df[s2_col].dtype == bool:
        mask = df[s2_col]
    else:
        numeric = pd.to_numeric(df[s2_col], errors="coerce")

        if numeric.notna().sum() > 0:
            mask = numeric.fillna(0) > 0
        else:
            mask = df[s2_col].astype(str).str.lower().isin([
                "true",
                "yes",
                "1"
            ])
    ...
    print(f"S2 available rows: {after}/{before}")
...
def main():
    ...
    for local_i, (_, row) in enumerate(df_batch.iterrows()):
        ...
        try:
            metadata["s2_image_time"] = ee.Date(
        ...
        try:
            metadata["s2_cloud_percentage"] = img.get(
