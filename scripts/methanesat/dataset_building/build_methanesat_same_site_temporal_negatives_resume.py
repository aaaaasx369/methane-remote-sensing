...
def args_parser():
    ...
    p.add_argument(
        "--restart",
        action="store_true",
        help="Ignore/delete only this script's checkpoint files and restart the search from zero.",
    )
    return p.parse_args()
...
def append_jsonl_fsync(path: Path, record: dict):
    """
    Append one completed record and force it to disk.
    If Python/Terminal is interrupted after this returns, this record remains.
    """
    path.parent.mkdir(parents=True, exist_ok=True)
    safe = {str(k): _json_safe(v) for k, v in record.items()}
...
def append_line_fsync(path: Path, line: str):
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("a", encoding="utf-8") as f:
...
def load_jsonl_checkpoint(path: Path) -> pd.DataFrame:
    ...
    with path.open("r", encoding="utf-8") as f:
        for lineno, line in enumerate(f, 1):
            ...
            try:
                ...
                # A hard power loss could leave one truncated final line.
                # Ignore only that broken line; all previous fsync'ed lines survive.
                print(f"[WARN] Ignoring truncated checkpoint line {lineno}: {path}")
...
def dedupe_checkpoint_rows(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return df

    # If a run was interrupted mid-positive, that positive is intentionally rerun.
    # Keep the newest record for the same positive/candidate combination.
    keys = ["positive_sample_id", "candidate_collection_id"]
...
def atomic_csv(df: pd.DataFrame, path: Path):
    """
    Never overwrite a good CSV with a half-written CSV.
    Write .tmp, fsync, then atomically replace final path.
    """
    path.parent.mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    checkpoint_jsonl = checkpoint_dir / "candidate_rows.jsonl"
    completed_txt = checkpoint_dir / "completed_positive_ids.txt"

    if args.restart:
        for p in [checkpoint_jsonl, completed_txt]:
            if p.exists():
                p.unlink()
        print("[RESTART] Search checkpoints cleared.")
        print("[RESTART] Existing raw imagery / downloaded data are NOT touched.")

    positives = load_positives(inv)
    atomic_csv(positives, outdir / "00_positive_sources.csv")

    completed_ids = load_completed_ids(completed_txt)
    checkpoint_existing = load_jsonl_checkpoint(checkpoint_jsonl)

    print("=" * 78)
    print("METHANESAT SAME-SITE TEMPORAL NEGATIVE SEARCH — RESUME SAFE")
    print("=" * 78)
    ...
    print("Already completed positives:", len(completed_ids))
    print("Checkpoint rows already on disk:", len(checkpoint_existing))
    print("Output:", outdir)
    ...
    for i, pos in positives.iterrows():
        ...
        if pid in completed_ids:
            print(f"[{i+1}/{len(positives)}] {pid} SKIP — already completed on disk")
            continue
        ...
        try:
            ...
            rec = {
                **pos.to_dict(),
                ...
                "candidate_class": "ERROR",
                "candidate_reason": f"L3_query_failed: {type(exc).__name__}: {exc}",
            }
            append_jsonl_fsync(checkpoint_jsonl, rec)
            append_line_fsync(completed_txt, pid)
        ...
        if not infos:
            rec = {
                **pos.to_dict(),
                "candidate_collection_id": "",
                "candidate_class": "NO_L3_HISTORY",
                "candidate_reason": "no_l3_image_covering_exact_coordinate",
            }
            append_jsonl_fsync(checkpoint_jsonl, rec)
            append_line_fsync(completed_txt, pid)
            completed_ids.add(pid)
        ...
        # Mark a positive as complete ONLY after all of its candidate collections
        # have been processed and fsync'ed. If interrupted mid-positive, it is
        # rerun on the next launch and duplicate rows are removed later.
        append_line_fsync(completed_txt, pid)
        completed_ids.add(pid)

        print(
            f"    CHECKPOINT SAVED — completed positives "
            f"{len(completed_ids)}/{len(positives)}"
        )

    # Build final tables from DISK checkpoints, not volatile RAM.
    allc = dedupe_checkpoint_rows(load_jsonl_checkpoint(checkpoint_jsonl))
    atomic_csv(allc, outdir / "01_all_same_site_l3_candidates.csv")
    ...
    atomic_csv(screened, outdir / "02_screened_temporal_candidates.csv")

    selected = select_strong(allc, args.max_negatives_per_positive, args.prefer_before)
    atomic_csv(selected, outdir / "03_selected_temporal_negatives.csv")
    ...
    atomic_csv(pd.DataFrame(ps), outdir / "04_positive_level_summary.csv")
    ...
    atomic_csv(pd.DataFrame(cov), outdir / "05_collection_coverage_summary.csv")
    ...
    lines = [
        ...
        "",
        "## Resume safety",
        f"- Completed positive IDs checkpointed: {len(completed_ids)} / {len(positives)}.",
        f"- Durable candidate checkpoint: {checkpoint_jsonl}.",
        "- Safe to interrupt and rerun with the same command; completed positives are skipped.",
        "- Do NOT pass --restart unless you intentionally want to rerun the search from zero.",
        "",
        "## Definition",
