...
def build_parser(defaults: Optional[Mapping[str, Any]] = None) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional row-level prediction CSV. The output preserves the "
            "evaluation manifest columns and appends logits, class "
            "probabilities, predicted_label, and correctness."
        ),
    )
...
def main(args: argparse.Namespace) -> dict[str, Any]:
    ...
    # Use the dataset's own dataframe as the source of row metadata so the
    # exported predictions follow exactly the same order as DataLoader rows.
    eval_rows = eval_ds.df.reset_index(drop=True).copy()
    if len(eval_rows) != len(eval_ds):
        raise RuntimeError(
            "Dataset/dataframe length mismatch: "
            f"len(eval_ds)={len(eval_ds)}, len(eval_ds.df)={len(eval_rows)}"
        )

    # The base dataset normally retries a corrupt sample by drawing a random
    # replacement row. That behavior is useful during training but would make
    # row-level prediction export ambiguous. For evaluation, fail loudly
    # instead of silently substituting another observation.
    if hasattr(eval_ds, "max_retries"):
        eval_ds.max_retries = 1
    loader = DataLoader(
        eval_ds,
    ...
    with torch.no_grad():
        for step, (x_dict, labels, sensors, sample_to_row) in enumerate(loader, 1):
            ...
            logits_cpu = fused_logits.detach().cpu()
            probabilities_cpu = torch.softmax(fused_logits, dim=1).detach().cpu()

            labels_all.extend(labels_cpu.tolist())
            preds_all.extend(preds_cpu.tolist())
            if fused_logits.shape[-1] == 2:
                scores_all.extend(probabilities_cpu[:, 1].tolist())

            batch_start = row_cursor
            batch_end = row_cursor + batch_rows
            batch_metadata = eval_rows.iloc[batch_start:batch_end].copy()

            if len(batch_metadata) != batch_rows:
                raise RuntimeError(
                    "Prediction/metadata alignment failed: "
                    f"requested rows [{batch_start}:{batch_end}], "
                    f"received {len(batch_metadata)} metadata rows."
                )

            label_column = getattr(eval_ds, "label_column", "label")
            if label_column in batch_metadata.columns:
                manifest_labels = (
                    pd.to_numeric(batch_metadata[label_column], errors="raise")
                    .astype(int)
                    .tolist()
                )
                if manifest_labels != labels_cpu.tolist():
                    raise RuntimeError(
                        "Prediction/metadata label mismatch. "
                        f"CSV labels={manifest_labels}, loader labels={labels_cpu.tolist()}"
                    )

            metadata_records = batch_metadata.to_dict(orient="records")
            for local_index, metadata_record in enumerate(metadata_records):
                true_label = int(labels_cpu[local_index].item())
                predicted_label = int(preds_cpu[local_index].item())

                record = dict(metadata_record)
                record.update(
                    {
                        "eval_row_index": int(batch_start + local_index),
                        "true_label": true_label,
                        "predicted_label": predicted_label,
                        "correct": bool(predicted_label == true_label),
                        "logit_negative": (
                            float(logits_cpu[local_index, 0].item())
                            if logits_cpu.shape[1] >= 1
                            else None
                        ),
                        "logit_positive": (
                            float(logits_cpu[local_index, 1].item())
                            if logits_cpu.shape[1] >= 2
                            else None
                        ),
                        "probability_negative": (
                            float(probabilities_cpu[local_index, 0].item())
                            if probabilities_cpu.shape[1] >= 1
                            else None
                        ),
                        "probability_positive": (
                            float(probabilities_cpu[local_index, 1].item())
                            if probabilities_cpu.shape[1] >= 2
                            else None
                        ),
                    }
                )
    ...
    if len(prediction_rows) != total_rows:
        raise RuntimeError(
            "Prediction row count mismatch: "
            f"predictions={len(prediction_rows)}, evaluated_rows={total_rows}"
        )

    if args.output_predictions_csv:
        prediction_output = Path(args.output_predictions_csv)
        prediction_output.parent.mkdir(parents=True, exist_ok=True)
        prediction_table = pd.DataFrame(prediction_rows)

        # Put the most useful audit and model-output columns first while
        # retaining every original manifest column for later emission-rate,
        # background, site, and temporal analyses.
        preferred_columns = [
            "eval_row_index",
            "id",
            "sample_id",
        ...
        result["predictions_csv"] = str(prediction_output)
